...
$list=['https://discord.com/api/channels/'.$result['result']['directmessage_channel']['id'].'/messages',];
if(isset($config['external']['discord']['webhook']['notice']['active'])&&!!$config['external']['discord']['webhook']['notice']['active']){
...
foreach($list as $k => $endpoint) {
	...
	$payload['embeds'][0]['fields'][] = [ 'name' => '接続元プロバイダー', 'value' => $result['result']['ipinfo']['as_name'].' '.$result['result']['ipinfo']['as_domain'].' (['.$result['result']['ipinfo']['asn'].'](https://ipinfo.io/'.$result['result']['ipinfo']['asn'].'))', 'inline' => false, ];
...
/*
* epoch秒をISO8601形式に変換して表示
* sql
SELECT "timestamp",TO_TIMESTAMP("timestamp");
*/
